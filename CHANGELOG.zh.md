...
- **全部面向用户的客户端文案支持英文本地化，语言跟随 DSH 自己的语言设置。** 内置 `zh` / `en` 两套文案并注册进宿主 `locale` 服务（0.1.5 的 Web 应用挂载 `@deepseek-ai/dsh-client-locale`，设置里有 Language 行）：显式选择优先，未设置时才回退浏览器语言；服务不可用（0.1.1 / 0.1.2 代际）时退回内置表 + 浏览器语言，且**不**把 `locale` 写进 `exports.inject` —— 缺失的注入会让整个 fiber 停在 INACTIVE。服务端兜底标签（未命名会话 / 其他项目 / 其他工作区）与 `SESSION_RUNNING` 错误码在客户端完成本地化，宿主行为不变。
- 移除了此前每次请求都发、宿主端却从不读取的 `x-dsh-locale` 请求头（宿主只认 `x-dsh-plugin` 这个 CSRF 头）。
- 验证：客户端用例 9/9（新增「宿主语言设置优先于浏览器」「命名空间不可用时回退内置表」两例）；全量 36/36 通过。

## [0.4.1] - 2026-09-16

### 修复

- **回收站里被宿主占用的会话显示「0 轮对话」、点「查看」内容空白。** `readSessionEvents` 调 `persistence.open(id)` 时漏传了访问模式：0.1.5 的 jsonl 后端按 `access === "read"` 分支，不传参即落进单写者路径（`claimWrite` + 独占租约），于是读取宿主仍持有写句柄的会话（用户刚删除、仍在应用里加载的那个）必然抛 `SessionAlreadyOwnedError`，又被静默吞掉 —— 列表轮数归 0、预览返回空，且只影响正在被查看的会话。现统一改用 `open(id, 'read')`；旧代际只有 `inspect`、没有 `open`，多传参数无副作用。读取失败现每会话告警一次，不再静默。
- 验证：真实 0.1.5-rc.2 包树夹具新增「宿主仍持有会话」阶段（修复前必然失败），18/18 通过；单测 32/32 通过（mock 现断言 `access === 'read'`）。
